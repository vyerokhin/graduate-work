...
three tasks is in general possible to perform perfectly we must choose a figure of merit to optimize,
...
For discrimination the competing figures of merit are the success rate of the measurement, the errors involved,
...
of the separation protocol.  For cloning, aside from successfully producing clones we are also interested in the quality of
the clones, and then the fidelity between clones and the original state is an important measure.  Because all quantum operations obey the same
set of conditions for evolution one may expect similar restrictions on disparate measurement strategies,
...
quantum systems as generalized measurements on density matrices representing the states' configurations.  Because the state is not a 
directly observable quantity we have prescribed methods of interacting with it.  Particularly the formulation due to Neumark
...
To perform a measurement in a quantum mechanical system, we must look to its state.
...
initial conditions.  In order to optimize the conditioned global fidelity of the clone states we must find the relationship between the quality of the clones and the likelihood of their occurrence. 
...
on input states and correlating them with the output states and occurrence probabilities thereof.
...
small changes in the error rate.  We notice from the optimized intermediate discrimination curve depicted in Fig. \ref{FigIM} that the trade-off for marginally increased
...
It is worthwhileto consider also the positivity of the $Q^i_c$, which would make it a real candidate for elimination.  Since $\omega_i - Q_{0,i} \geq 0$ we analyze the positivity of $\omega_i Q_0 - Q_{0,i}$ . For this to be positive we need  $Q_0 \geq \bar{ Q_{0,i}}$ which means that the UD failure rate of that normalized subspace should be smaller than the total UD failure rate of the system of subspaces. 
...
\end{array}%
$%
\caption[ $\quad$ Three Subspace Interpolation]{NEEDS CAPTION}
\label{fig:Graphs}
\end{figure}
...
\caption[ $\quad$ Subspace Intersections]{NEEDS CAPTION}
\label{fig:Cont}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%SSD%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
...
We notice that with more subspaces, the condition for the overlap region of SSD over the POVM does not change for individual subspaces as the bar transformation would show us. We show this structure Fig. \ref{fig:SSD}, where the shaded regions represents the SSD domains.
...
Since any quantum transformation must obey unitary evolution, the unitarity equation places strict bounds on a system coupled with an ancilla in hopes of increasing its overlap.  This includes all possible transformations no matter the protocol or designation.  Therefore we should expect that the equations for state separation coincide with the studied measurement strategies in particular bounds.  
...
In terms of this conic geometry,  optimality is again given by a tangency point, this time between ellipses and parabolas. Because of the features of these families of conics, these points of tangency necessarily lie in the region between their envelopes,  which is the gray area in Fig.~\ref{fig:3}. Fig.~\ref{fig:3} also illustrates optimality. Given a maximum failure rate $Q_{\rm max}$ and some initial overlap~$s$ ($Q_{\rm max}=0.45$ and $s=0.6$ in the example considered in the figure), we plot the corresponding ellipse defined by Eq.~(\ref{obj fun conic}) Among the various parabolas, characterized by the final overlap $s'$ the one that minimizes~$s'$  has a unique point of tangency with the ellipse, thus giving us the solution, $s'_{\rm min}$.  To keep the notation simple we will drop the subscript ``$\rm min$'' wherever no confusion arises. 
...
\chapter{Cloning Pure States}
 ...
 The interest of quantum systems comes from their intrinsically probabilistic nature. If perfect cloning of quantum states were possible, this uncertainty would vanish, thus providing an intuitive argument for why perfect cloning is not deterministically feasible.  There have been two different strategies for dealing with the problem of cloning states:  one is to always produce clones that are 'as close' to the originals as possible, usually by means of global fidelity, as was first done for arbitrary states \cite{Hillery and Buzek} and then for one of two known pure states \cite{Bennett}.  
...
has been derived by Chefles \textit{et al.}. The authors
...
\bibitem{Chefles Separation} Anthony Chefles and Stephen M Barnett 1998 J. Phys. A: Math. Gen. 31 10097
