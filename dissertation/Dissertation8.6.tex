...
and their relationships, are discussed.  Via Neumark's theorem, 
our description of these unitary processes can be implemented using linear optical devices.
...
directly observable quantity we have a prescribed method of interacting with it.  Particularly the formulation due to Neumark
where the evolution of the state of a quantum system can be described by the action of a unitary matrix.  This unitary can be decomposed into
measurement operators responsible for different outcomes.  We will be particularly interested in three subtopics.  The optimal strategies for discrimination of quantum states, cloning of quantum states, and separating them will be discussed.
...
In the subsequent chapter we describe the IM solution two different ways. 
...
We then apply the intermediate transformation solution to several classes of mixed states.  For the first, the two mixed states  lie in a Jordan basis with respect to one another.  This structure simplifies the mixed-state problem because we can apply the previous two-dimensional solution to each subspace in our Hilbert space.  The solution is a normalized version of the known result.  However an extra optimization is required to find the optimal distribution of failure over the subspaces and this final optimization leads to an interesting threshold structure. When the failure rate is decreased from the UD limit we reach a critical value when one subspace's failure rate reaches zero and it is excluded from the optimization.  Subsequently all remaining subspaces are removed one after the other as we continue to decrease the failure rate, requiring a re-optimization at each epoch.  As we reach the ME boundary only a single subspace has any failure result.  We thoroughly analyze this threshold structure, considering points of continuity, intersection, and limits of the regime. The other class of mixed states we consider are qubits.  Again a subsequent optimization is required after the initial transformation is applied.  This optimization is non-trivial.  While numerical results can be obtained for all ranges of solutions, only in particular cases we obtain analytic results.

We also consider applying the MC framework to high dimensional mixed states to generalize the role of detection operators.  Instead of associating a single detection operator for a given state, we may additionally add the discriminating factor of confidence to a class of operators for each state.  The advantage of this formulation is that each measurement operator is extracting information at the optimal level of confidence for the corresponding eigenvector in the confidence space of the density matricies.  This allows us to expand the success rate of the UD regime in higher dimensional spaces and have a higher confidence level for some measurement results over any interpolative strategy of the type described earlier.


It is a straightforward logical conclusion that if perfect quantum state discrimination is impossible, then so too should perfect quantum cloning be impossible.  Were it possible, we would clone an infinite copies and conduct perfect discrimination on any system.  The original proofs are by Wooters and Dieks \cite{Wooters, Dieks}.  Strangely, the impossibility of cloning leads to advantages to quantum based communications protocols of practical relevancover classical ones.  A common example is provable security in quantum key distribution, with recent works showing more applications~\cite{Pomarico, Bart}.  Developments in cloning, including deterministic but approximate cloning~\cite{Buzek1,Gisin1,Buzek2,Brub,Chefles1,Fiurasek} and probabilistically perfect cloning \cite {DuanGuo,Fiurasek1,Muller} provide anchors for better understanding quantum theory as a whole, such as the relationship between the no-cloning and no-signaling theorems~\cite{Barnum}, and  fundamental limits on quantum measurements~\cite{Chiribella,Bae,Chiribella2006,Gendra}. In particular, cloning's relationship with the fundamental limits of state discrimination will be central to this Letter. For reviews citing recent developments, applications and experiments related to cloning see~\cite{review1,Fan} and to state discrimination see \cite{Bergou}.

When knowledge of the state preparation is available, perfect cloning is probabilistically possible. With the first result in probabilistic cloning, Duan and Guo \cite{DuanGuo} considered the problem of producing perfect clones of linearly independent pure states and focused on the two state case.  They found the maximum average success rate when both states are equally likely, the case of equal priors, and set this success probability as lower bound for arbitrary prior probabilities.  While other work has been done on this problem, there has until now been no general solution. In our first result on cloning we obtain the general analytic solution and, with its help, examine the relationship of cloning and state discrimination.  Subsequently we solve the approximate probabilistic cloning problem for the same
initial conditions.  In order to optimize the conditioned global fidelity of the clone states we must find the relationship between the quality of the clones and the liklihood of their occurance. 

To this end we develop a theory of optimal state separation, which is a generalization of the first step of intermediate state discrimination or cloning, where the incoming pure state is probabilistically made more distinguishable from its counterpart.  In fact any probabilistic transformation acting on a two-state family can be characterized by the change in separation of the two states.  Whether the map is physical for a given failure rate is entirely dependent on the state separation problem. Chefles and Barnett \cite{Chefles1} first considered this relationship for equal prior probabilities, so we extend their work to the general a-priori region using two distinct geometric methods.  For the first we generalize the parametrization devised for perfect probabilistic cloning to describe the optimal rate of failure for a fixed separation.  Then we parametrize the relevant equations so that the optimal point is described by an intersection of two conic sections: a parabola and an ellipse.  This parametrization allows us several different perspectives on the problem.  For example, if we wish to maximimize separation for a given failure rate, or to see the tradeoff between separation and failure, the parametrized system of equations of the conic sections allows us to solve for any two variables in terms of the other parameters of the problem.

We conclude by demonstrating the utility of the Neumark method by explicitly describing the action of unitary transformations
on input states and correlating them with the output states and occurance probabilities thereof.
...
We design linear optical experiments for actualizing this unitary by the methods described in \cite{Reck, Sun, BergouImp}.
Namely, we decompose the unitary into the action of beamsplitters on beams of photons.  We provide implementations for optimal intermediate discrimination, for deterministic and  probabilistic approximate  cloning, and for state separation.  Our solution to the probabilistic approximate implementation is a two step procedure built from first a separation machine followed by a deterministic cloner. As this method does not require more advanced experimentalist technology than has been implemented for the realization of the UD experiments, we are confident that this theory can be readily verified.
...
We begin this by describing pure quantum states $\psi$ as vectors in a Hilbert space and an ensemble of states as
...
The Kaus operators can be associated with measurement operators $\Pi_i$ such that $\Pi_i = A^\dagger_i A_i$. Hence the $\Pi_i$ are a decomposition of the identity in terms of positive semi-definite matrices.  A measurement can be either a projector onto an eigenvector of the Hilbert space or a generalized measurement (POVM).  In the latter case it must fulfill only 2 properties:
\begin{enumerate}
\item $\sum \Pi_i = 1$
...
Consider the problem of discriminating between two pure states $\ke \psi_1$ and $\ke \psi_2$ that are sent one at a time with a known probability distribution.  By Neumark's theorem the Hilbert space of our states is two dimensional. Therefore the measurement space is also two dimensional allowing for two orthogonal outcomes, one associated with each state. If we could always perform this perfectly then we should be able to write a unitary U such that if we apply this unitary to our incoming states, we always turn them into orthogonal vectors: 
\begin{eqnarray*}
U\ke {\psi_1}= \ke 1,\\
U\ke {\psi_2} = \ke 2.
\end{eqnarray*}
The states $\ke 1$ and $\ke 2$ are orthogonal in the basis of the input states, $\bk 1 2 = 0$, and each result is associated with the respective input.  However
...
functions in order to perform a physical measurement.  We must choose a unitary that performs this task optimally according to some figure of merit, typically a probability measure.  For all future discussion 
...
small changes in the error rate.  This motivates an intermediate strategy where we wish to perform the best measurement for a
fixed rate of failure.  By varying the failure rate from $0$ to $Q_c$ we should recover both the ME and UD results in a solution that minimizes error
...
\protect\caption[$\quad$ Optimal Failure Rate vs A-Priori Probability for UD]{Failure rate $Q$ vs a-priori probability $\eta_1$, with the maximum failure $Q_c$ Eq.(\ref{eq:Qmax}) and boundary $Q_{\rm b} = 2 \eta_1 \eta_2 \sin^2 \theta/(1-Q_0)$ between the two measurement regimes I and III, and the POVM regime II.}
\label{Fig1} 
\end{figure}
...
\begin{eqnarray}
U \ke {\psi_1}= \sqrt{p_1} \ke 1 + \sqrt{r_1} \ke 2 + \sqrt{q_1} \ke 0 \\ \label{IM Unitaries}
...
We first find the partial derivaties  $ \partial F/ \partial r_i = 0$, then solve for the error rates $r_i$ in terms of the Lagrange multiplier $\lambda$.  Substituting the error rates into the overlap constraint allows us to solve for $\lambda$, which gives us the individual error rates after a lot of algebra as
\begin{eqnarray}
...
\end{eqnarray}
where $\alpha_i \equiv 1-q_i $ and  $\omega\equiv(s -\sqrt{q_1 q_2})^2$. These are not the final result because they were optimized individually. Let us put these into the expression of minimum error and do a final optimization. Then
...
\section{Maximum Confidence Measurements}
  We begin from the work of Sarah Croke \textit{et al.} 2006 and of Ulrike Herzog, dated from February 2009 on maximum confidence.  There two mixed states were measured using 3 POVM's including one failure operator.  The final successful measurement $\Pi_i$ was a normalized, shortened projector onto the eigenket corresponding to the largest eigenvalue of the normalized density matrix $\widetilde{\rho_i}$

Mathematically, given $ \rho_1$ and $\rho_2$, the density matricies of the two states to be distinguished and their a-priori probabilities $\eta_1$ and $\eta_2$, the MC formalism tells us to find:
...
\[  \widetilde{ \Pi_i} = \frac{ \rho^{\frac{1}{2}}\Pi_i  \rho^{\frac{1}{2}}}{Tr \rho \Pi_i}, \]
 ...
 \[\widetilde{\rho_1} = v_{max}\vert v_1 \rangle \langle v_{1} \vert + v_{min}\vert v_N \rangle \langle v_N \vert + \sum^{N-1}_{n=2} v_{n}\vert v_n \rangle \langle v_n \vert.\] Conveniently $\widetilde{\rho_1} = I - \widetilde{\rho_2}$, and we assume that the maximum and minimum eigenvalues are not degenerate.  The measurement operators take the form
\begin{eqnarray}
...
\end{equation}
  The failure operator $\Pi^{\frac{1}{2}}$ is hence only neccessary for the range of the spectrum in which both states are equally likely, and the new  measurements for $\rho_1$ and $\rho_2$ replace what would otherwise have been less-than-maximum-confidence spaces sorted into the failure operator.

We demonstrate this formalism with a few simple examples.  Suppose we wish to discriminate between two mixed states in 3 dimensions with 1 dimensional kernels.  Suppose 
\begin{eqnarray*}
...
\end{equation*}
and $C_1 = C_2 = 1$ implies unambiguous discrimination. We attain the success probability of the measurement as
...
and again $C_1 = C_3 = 1$ for the projectors onto the kernels while $C_2 = \frac{8}{11}$ is the confidence of the newly-interpreted operator
...
The extra conclusive measurements give us an additional success rate as $ P'_s = \eta_1 Tr(\rho_1 \Pi^{\frac{4}{5}}_1) + \eta_2 Tr(\rho_2 \Pi^{\frac{4}{5}}_2) =  \frac{16}{35}$, with total success probability being the sum of the individual rates and a failure rate significantly lower than $Q_UD$:
\[P^{tot}_s = P_s + P'_s = \frac{26}{35}, \quad P'_{e} = \frac{4}{35},\quad Q' = \frac{5}{35}.\]

Now we apply the theory to existing situations, beginning with the simplest, two symmetric pure states in 2d.  First we show the example of an attempt to improve the success probability for IDP measurements, where the input states are $\vert \psi_1 \rangle = cos \theta \vert 0 \rangle + sin \theta \vert 1 \rangle $, $ \vert \psi_2 \rangle = cos \theta \vert 0 \rangle - sin \theta \vert 1 \rangle$.  The well known measurements take the form
\begin{eqnarray*}
...
Finally we look at the last example from \cite{Herzong} for a case with higher-dimensional operators:
...
This gives the failure rate as  \[Q^{MC} = 1-\frac{2m}{d}p \cos \gamma^{max} \]
and confidences 
...
So we've found an effective intermediate strategy that maximizes the confidence and then success rate of the measurement by utilizing more confidences for each state.
...
\chapter{Cloning Pure States}
 ...
 The interest of quantum systems comes from their intrinsically probabilistic nature. If perfect cloning of quantum states were possible, this uncertainty would vanish, thus providing an intuitive argument for why perfect cloning is not determinstically feasible.  There have been two different strategies for dealing with the problem of cloning states:  one is to always produce clones that are 'as close' to the originals as possible, usually by means of global fidelity, as was first done for arbitrary states(\cite{Hillery and Buzek 22}) and then for one of two known pure states ( \cite{Bennett}, etc).  

The other stategy is to sometimes make perfect copies, at the cost of sometimes failing to do so.  The seminal paper by Duan and Guo (\cite{DuanGuo}) solved the problem of making perfect clones probabilistically for equal a-priori conditions, and set this success probability as an upper bound for general a-priori probabilities. While other work has improved on this bound ( ), there has until now been no general solution.  Other perfect probabilistic cloning schemes focus on other types of states ().

 Work has also been done on intermediate strategies where fidelity is maximized for a certain rate of failure, either pure states (\cite{Chefles}) or mixed (\cite{Fiurasek, Setaski}). There are practical reasons for wanting to produce perfect clones (\cite{applications}), with applications in communication and computing. Experiments have been suggested (\cite{Fiurasek1}) and implemented (\cite{Muller}) for cloning quantum states.
...
has been derived by Chefles \textit{et al.} \cite{Chefles1999}. The authors
...
\end{equation}
To find its minimum we use two equations; for the first we differentiate $\frac{\partial \Lambda_{\rm min}}{\partial u} = \frac{\partial v}{\partial u}$ and for the second $\Lambda_{\rm min}=v$ to get two master equations that we can solve for any two parameters in terms of $\theta$. 
\begin{eqnarray*}
...
\bibitem{Fiurasek} Jaromir Fiurasek and Miroslav Jezek, Phys. Rev.
...
\bibitem{Bart} K. Bartkiewicz, A. {C}ernoch, K. Lemr, J. Soubusta, and M. Stobinska, Phys Rev. A {\bf 89}, 062322 (2014).
%Efficient amplification of photonic qubits by optimal quantum cloning
\bibitem{Buzek1} V. Buzek and M. Hillery, Phys Rev. A {\bf 54}, 1844 (1996).
...
\bibitem{Buzek2} V. Bu{z}ek and M. Hillery, Phys. Rev. Lett. \textbf{81}, 5003 (1998).
...
and J. A. Smolin, Phys Rev. A {\textbf 57}, 2368 (1998).
...
\bibitem{Fiurasek} J. Fiur{\' a}{\v s}ek, S. Iblisdir, S. Massar, and N. J. Cerf, Phys Rev. A \textbf{65}, 040302(R) (2002).
...
\bibitem{Fiurasek1} J. Fiur{\' a}{\v s}ek, Phys Rev. A \textbf{70}, 032308 (2004).
